    ...
    try {
        ...
        if (x64AsarMode === asar_utils_1.AsarMode.NO_ASAR) {
            ...
            if (!comparison.same) {
                ...
                await fs.copy(path.resolve(opts.x64AppPath, 'Contents', 'Resources', 'app'), path.resolve(tmpApp, 'Contents', 'Resources', 'app-x64'));
                await fs.remove(path.resolve(tmpApp, 'Contents', 'Resources', 'app'));
        ...
        for (const plistFile of plistFiles) {
            ...
            const plistFilePath = path.resolve(tmpApp, plistFile.relativePath);
            if (!(await fs.pathExists(plistFilePath))) {
                debug_1.d('creating directory for Info.plist path', plistFilePath);
                await fs.ensureDir(path.dirname(plistFilePath));
            }
            await fs.writeFile(plistFilePath, plist.build(mergedPlist));
        }
