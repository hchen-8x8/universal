  ...
  try {
    ...
    for (const file of x64Files.filter((f) => f.type === AppFileType.PLAIN)) {
      ...
      if (x64Sha !== arm64Sha) {
        ...
        if (
          ...
          path.basename(file.relativePath) === 'CodeResources'
        ) {
    ...
    for (const machOFile of x64Files.filter((f) => f.type === AppFileType.MACHO)) {
      ...
      if (x64Sha === arm64Sha) {
        ...
        opts.skipMergeFiles !== undefined &&
        minimatch(machOFile.relativePath, opts.skipMergeFiles, { matchBase: true })
      ) {
        continue;
      }
    ...
    for (const plistFile of plistFiles) {
      if (plistFile.relativePath.indexOf('/app') === -1) {
